  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
        ...
        >
          <button onClick={() => setOrderTrackerOpen(false)}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <span className="text-xs font-semibold text-sky-400 uppercase tracking-wider">Живой статус заказа</span>
              <h3 className="text-2xl font-black text-slate-50">Заказ #{activeOrder.orderNumber}</h3>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 p-4 bg-slate-950/80 border border-slate-800 rounded-2xl">
            ...
            <div className="text-center">
              ...
              <span className="text-3xl font-black text-white font-mono tracking-widest">{elapsed}</span>
            </div>
          </div>
            ...
            {steps.map((step, idx) => {
              ...
              return (
                <div key={step.id} className="relative flex items-start space-x-4">
                  <div className={`absolute -left-[25px] top-0 w-6 h-6 rounded-full border-2 flex items-center justify-center ${isDone ? 'bg-red-600 border-red-400 text-white' : 'bg-slate-900 border-slate-700 text-slate-600'}`}>
                    <Icon className="w-3.5 h-3.5" />
                  </div>
                  <div>
                    <h4 className={`text-sm font-bold ${isCurrent ? 'text-red-400 font-extrabold text-base' : isDone ? 'text-slate-200' : 'text-slate-500'}`}>
                      {step.label}
                    </h4>
          ...
          <div className="pt-4 border-t border-slate-800 flex items-center justify-between text-xs">
            ...
            <a href="tel:+79308184040" className="flex items-center gap-1 font-bold text-red-400 hover:underline">
