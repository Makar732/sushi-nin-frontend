...
import { X, Truck, Store, CreditCard, Banknote, Smartphone, Clock, MapPin, CheckCircle2, Loader2, Sparkles } from 'lucide-react';
...
export const CheckoutModal = () => {
  ...
  const onSubmit = async (data: CheckoutFormData) => {
    ...
    const generatedOrderId = `SM-${Math.floor(100000 + Math.random() * 900000)}`;
    const fullAddress =
    ...
    try {
      ...
      if (json.success) {
        ...
        try {
          ...
          // Fallback if confetti script fails
        }
          ...
          <div className="p-5 border-b border-slate-800 bg-slate-900/90 flex items-center justify-between shrink-0">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">«СушиМин» — Доставка еды в Заволжье</p>
              </div>
            </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        ...
                        className={`p-3.5 rounded-2xl border flex items-center space-x-3 transition ${
                          deliveryType === 'delivery'
                            ? 'bg-red-600/20 border-red-500 text-slate-100 shadow-md shadow-red-500/10'
