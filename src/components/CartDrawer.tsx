...
import {
  X, Trash2, Plus, Minus, ShoppingBag, Truck, Tag,
  ArrowRight, AlertCircle, Gift, Check, Loader2,
...
// ── Лёгкий внутренний toast — не требует внешних зависимостей ────────────────
interface SyncToastProps {
  messages: string[];
  onClose: () => void;
}

const SyncToast: React.FC<SyncToastProps> = ({ messages, onClose }) => {
  useEffect(() => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -16 }}
      className="mx-4 p-3 bg-amber-500/15 border border-amber-500/40 rounded-2xl space-y-1"
    >
      <div className="flex items-center justify-between mb-1">
        <span className="flex items-center gap-1.5 text-xs font-extrabold text-amber-300">
          <AlertTriangle className="w-3.5 h-3.5 shrink-0" />
          Цены в корзине обновлены
        </span>
        <button
          onClick={onClose}
...
export const CartDrawer = () => {
  ...
  const handleSyncPrices = useCallback(async () => {
    ...
    try {
      ...
      if (result.hasChanges && result.messages.length > 0) {
        setSyncMessages(result.messages);
        setShowSyncToast(true);
      }
    } catch (err) {
      console.error('[CartDrawer] syncCartPrices failed:', err);
    } finally {
      setIsSyncing(false);
    }
  }, [isSyncing, cart.length, syncCartPrices]);

  // При открытии корзины — сразу синхронизируем цены с сервером
  useEffect(() => {
          ...
          >
            ...
            <div className="p-4 sm:p-6 border-b border-slate-800 bg-slate-900/90 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-2">
                {/* Кнопка ручной синхронизации цен */}
                {cart.length > 0 && (
                  <button
                    onClick={handleSyncPrices}
                    disabled={isSyncing}
                    title="Обновить цены"
                    className="p-2 text-slate-500 hover:text-sky-400 rounded-full hover:bg-slate-800 transition disabled:opacity-40"
                  >
                    <RefreshCw
            ...
            {/* Toast: изменение цен */}
            <AnimatePresence>
              {showSyncToast && syncMessages.length > 0 && (
                <div className="pt-3">
                  <SyncToast
                ...
                <>
                  <div
                    className={`flex items-center justify-between text-xs font-semibold pt-1 ${
                      giftUnlocked ? 'text-amber-400' : 'text-slate-500'
                    }`}
                  >
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3">
              {/* Индикатор синхронизации поверх списка */}
              {isSyncing && (
                <div className="flex items-center justify-center gap-2 py-2 text-xs text-sky-400">
